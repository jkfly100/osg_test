...
inline void travelPath::GeoAdd(int i)//添加geopoint
{
	...
	GeoPoint geo(_mapNode->getMap()->getSRS(), _position[i].first.first, _position[i].first.second, _position[i].second, ALTMODE_ABSOLUTE);
...
inline void travelPath::matrixTrans()//设置剩余路径点
{
	...
	while (j < _numPos - 1)
	{
		...
		gMatrix.preMultRotate(osg::Quat(osg::DegreesToRadians(-60.0), osg::X_AXIS));
		osg::Quat q;
		gMatrix.get(q);
		_quat.push_back(q);
...
inline void travelPath::setPath2()
{
	...
	_animationPath->setLoopMode(osg::AnimationPath::LOOP);//设置路径是回摆的
	...
	_viewer->setCameraManipulator(_animationPathMp);
}
